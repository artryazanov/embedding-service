...
async def websocket_worker_task():
    if not settings.reverb_app_key or not settings.reverb_host:
        logger.warning("WebSocket disabled: REVERB_APP_KEY or REVERB_HOST not configured.")
        return
    ...
    while True:
        try:
            ...
            async with websockets.connect(uri) as websocket:
                ...
                try:
                    async for message in websocket:
                        ...
                        if "VectorizeTaskEvent" in event or "BatchVectorizeTaskEvent" in event:
                            ...
                            try:
                                if "BatchVectorizeTaskEvent" in event:
                                    result = engine.encode(payload.get("items", []), batch_size=64)
                                else:
            ...
            logger.error(f"WebSocket failure: {e}. Retrying in {retry_delay} seconds...")
            await asyncio.sleep(retry_delay)
            retry_delay = min(retry_delay * 2, 60.0) # Exponential backoff up to 60 seconds
